...
class WriterStructuredOutput(BaseModel):
    ...
    #chapter_summary: str = Field(description = "A summary of the chapter with the key points and explanation of them.")
...
class ApprovedBrainstormingIdea(BaseModel):
    feedback: str = Field(description = "The corresponding critique or feedback if further improvements are needed")
    grade: int = Field(description = "The overall grade assigned to the draft idea based on the criterias")

class State(TypedDict):
    content: Annotated[List[str], operator.add]
    chapter_titles: Annotated[List[str], operator.add]
...
def _get_model(config: GraphConfig, key:Literal['instructor_model','brainstormer_model','critique_model','writer_model'], temperature:float, default:Literal['openai', 'google','meta']='openai'):
    ...
    if model == "openai":
        ...
        return ChatGoogleGenerativeAI(temperature=temperature, model_name="gemini-1.5-pro-exp-0801")
    elif model == 'meta':
        return ChatGroq(temperature=temperature, model_name="llama-3.1-70b-versatile")
    else:
        raise ValueError
